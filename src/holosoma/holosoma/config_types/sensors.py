...
class MujocoCameraConfig:
    ...
    that set a given flag must agree (validated in ``SensorsConfig.validate_warp_render_flags``).
...
class CameraSensorConfig:
    ...
    Keyed in :attr:`SensorsConfig.cameras` by its sensor name — the handle for
    ...
    def validate_camera(self) -> CameraSensorConfig:
        """Positive resolution, known data types, at least one modality."""
...
@dataclass(frozen=True, config=_FORBID_EXTRA)
class SensorsConfig:
    """Composition of perception sensors for the simulator.

    Cameras are produced here; how their frames leave the sim (live window, mp4, ROS2) is handled by
    the ``sensor_egress:`` presets. The MuJoCo spectator viewer draws each camera's frustum when
    ``simulator.config.debug_viz`` is on.
    """

    cameras: dict[str, CameraSensorConfig] = field(default_factory=dict)
    """Mounted cameras to create, keyed by sensor name (the ``get_camera_data`` key). Empty
    (default) means no sensors."""

    @model_validator(mode="after")
    def validate_camera_mounts(self) -> SensorsConfig:
        """Reject an ``actor`` mount that names ``"robot"``; the robot is addressed via the
        ``robot_link`` kind. (Names are unique by construction — they are dict keys.)
        """
        for name, c in self.cameras.items():
            if c.mount.target_kind == "actor" and c.mount.target == "robot":
                raise ValueError(
                    f"Camera '{name}' mounts on actor 'robot'; use target_kind='robot_link' "
                    f"(name the robot link, e.g. 'pelvis') for the robot."
                )
        return self

    @model_validator(mode="after")
    def validate_warp_render_flags(self) -> SensorsConfig:
        """Reject conflicting MuJoCo-Warp appearance flags across cameras.

        ``use_shadows``/``use_textures``/``use_precomputed_rays`` are global to the shared Warp
        render context; cameras that set a given flag must agree, ``None`` imposes no constraint."""
        for attr in ("use_shadows", "use_textures", "use_precomputed_rays"):
            setters = {name: getattr(c.mujoco, attr) for name, c in self.cameras.items() if c.mujoco is not None}
