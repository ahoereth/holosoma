...
class TerrainLocomotion(TerrainTermBase):
    ...
    def _get_env_origins(self):
        ...
        if self._cfg.spawn.randomize_tiles:
            ...
            # Eval mode: all robots at tile (0,0) for deterministic evaluation. Use the
            # terrain's origin-grid accessor (not ._env_origins directly) so OBJ terrains,
            # which derive origins from the loaded mesh and never populate ._env_origins,
            # work in eval as they already do in training via sample_env_origins.
