...
def motion_ends(env, command_name: str = "motion_command", **_) -> torch.Tensor:
    """Time out at the final frame of each environment's selected motion clip."""
    ...
    end_steps = motion_command.motion.motion_end_idx[motion_ids].to(device=motion_command.time_steps.device)
    return motion_command.time_steps >= end_steps - 1
