...
class BasicSdk2Bridge(ABC):
    ...
    def _get_base_odometry(self):
        ...
        (body) frame via the same ``quat_rotate_inverse`` helper the IMU gyro and the ROS2
        odometry egress use, matching the real robot's ``SportModeState`` (``rt/odommodestate``)
        whose ``velocity`` twist is body-frame.
