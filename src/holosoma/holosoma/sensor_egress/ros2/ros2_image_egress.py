"""ROS2 image-publishing egress: publishes rendered sim camera frames as sensor_msgs/Image or
CompressedImage (+ optional latched CameraInfo), one node fanning out to many camera routes.

This is the only module in the egress system that touches ROS2. It is imported solely via
``ROS2ImageEgressConfig.egress_cls`` (deferred), so the rest of holosoma stays importable without
...
egress does. All non-ROS logic (encoding, K-matrix, drop-oldest queue) lives in sibling ROS-free
...
from holosoma.sensor_egress.base import CameraIntrinsics, SensorEgress
from holosoma.sensor_egress.ros2.camera_info import camera_info_from_intrinsics
from holosoma.sensor_egress.ros2.encode import EncodedImage, encode_frame
from holosoma.sensor_egress.ros2.worker import PublishWorker
...
class ROS2ImageEgress(SensorEgress):
    """One ROS2 node publishing the configured camera routes."""

    config: ROS2ImageEgressConfig

    def __init__(self, config: ROS2ImageEgressConfig, simulator: BaseSimulator) -> None:
