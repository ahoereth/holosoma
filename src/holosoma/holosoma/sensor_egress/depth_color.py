"""Depth-map colorization shared by the sensor egresses (ROS-free; cv2 deferred).

Turns a float32 depth map (meters, ``+inf`` no-hit) into an ``HxWx3`` uint8 RGB image using a
named OpenCV colormap. Both egresses consume this: the viz egress tiles the result into its grid,
and the ROS2 egress colorizes a depth route before wire-encoding it as RGB (jpeg/png/rgb8). It lives
here — neutral ground under ``sensor_egress`` — so neither egress flavor imports the other's package.
...
assert set(_CV2_COLORMAP_NAMES) | {"gray"} == set(_DEPTH_COLORMAPS), (
    f"_CV2_COLORMAP_NAMES {sorted(_CV2_COLORMAP_NAMES)} + 'gray' must match "
    f"_DEPTH_COLORMAPS {sorted(_DEPTH_COLORMAPS)}"
)
