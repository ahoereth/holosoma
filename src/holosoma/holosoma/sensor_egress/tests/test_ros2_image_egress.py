"""Offline behavior tests for ROS2ImageEgress message construction (no DDS, no sim, no spin).
...
class _FakeSimulator:
    """Minimal stand-in exposing what the egress reads off the simulator (no backend, no DDS)."""

    def __init__(self, *cams):
        self.sensor_config = SensorsConfig(cameras=dict(cams))
        self.num_envs = 1
        self.headless = True
...
def fake_ros(monkeypatch):
    """Monkeypatch the rclpy pieces ``ROS2ImageEgress.start`` uses so no real node/spin/DDS runs.
...
def _make(cfg, *cams):
    """Construct the egress with a fake simulator built from the given cameras."""
    return cfg.egress_cls(cfg, _FakeSimulator(*cams))
